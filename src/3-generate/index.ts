...
export async function run(ctx: PipelineContext): Promise<void> {
  ...
  if (!ir) {
    ctx.log.error(
      'No validated or aggregated IR found in pipeline state. Extract/validate stages may not have run.',
    );
    throw new Error('Missing IR - extraction stage must run before generation');
  }
  ...
  if (outputs.length > 0) {
    ...
    // If multiple generators, concatenate them (or write to separate files if dsl_out is a directory)
    const content =
      outputs.length === 1
        ? outputs[0].content
        : outputs.map((o) => `# Generated by ${o.name}\n\n${o.content}`).join('\n\n');

    ctx.log.debug(`Writing DSL output to ${outputPath}`);
